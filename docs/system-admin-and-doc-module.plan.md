---
name: system-admin-and-doc-module
overview: 系统管理模块和文档管理模块需求合并文档
todos: []
---

# 系统管理模块和文档管理模块需求合并文档

本文档合并了系统管理模块和文档管理模块的完整需求。
...
- 配置项清单：模型配置、重排序模型、向量化模型（2层：系统/租户）、向量库（2层：系统/租户）、可上传文件类型、文件上传大小限制、文本切分策略、相似度、TopK等；配置项需要分类管理（模型相关、存储相关、文档相关、问答相关等），分组展示
...
- 用户配置查看：文件列表中显示每个文件使用的配置项；用户可以查看自己之前为不同文件配置的配置项历史，不可以修改；用户查看配置项历史不需要权限控制，所有用户都可以查看
- 配置项的删除和重置：不可以删除或重置
- 配置项的版本管理：系统管理模块需要历史记录（数据级的，业务上没关系），不支持回滚；文档管理模块不需要版本管理
...
- 配置项的审计日志：只针对特定配置项，管理员、租户记录，用户不记录
- 权限：平台管理员、租户管理员可以访问配置管理模块；普通用户配置用户级配置项需要权限控制，租户管理员配置用户可配置（粗项），配置项中要关联角色，及这个配置项哪些角色可以配置；默认所有的用户都可以配置；权限码命名规范（如 system:config:read、system:config:update），不做系统级和租户级区分
...
- 配置项归属：上传类型（可上传文件类型）、文件上传大小限制、文本切分策略在系统管理的配置管理模块中统一管理；文本切分策略属于配置管理模块，文档管理模块读取和使用系统/租户级配置，用户可在文档管理模块中配置用户级配置项
...
- 上传类型：初始白名单 txt、md、pdf、word；文件上传大小限制三层配置（系统/租户/用户），单文件默认50M，总容量不限制
...
- 用户配置权限：需要权限控制，租户管理员配置用户可配置（粗项），配置项中要关联角色，及这个配置项哪些角色可以配置；某些特定的配置项普通用户看不到
- 用户配置查看权限：用户查看配置项历史不需要权限控制，所有用户都可以查看
- 配置项的删除和重置：不可以删除或重置
- 审计：上传/下载/删除/标签变更日志（用户、租户、时间、结果、IP/UA）；标签删除记录操作日志；配置项的审计日志：只针对特定配置项，管理员、租户记录，用户不记录
...
以下规则统一适用于系统管理模块和文档管理模块的所有配置项：

1. **配置项的删除和重置**：不可以删除或重置
2. **配置项的版本管理**：系统管理模块需要历史记录（数据级的，业务上没关系），不支持回滚；文档管理模块不需要版本管理
3. **配置项的验证规则**：预留接口以及空方法，这次先不做
4. **配置项的导入导出**：所有导出的都预留，不做
5. **配置项的备份和恢复**：不可以
6. **配置项的迁移**：没有迁移功能
7. **配置项的依赖关系**：不依赖
8. **配置项的冲突处理**：使用低级的覆盖
9. **配置项的查询和统计**：租户管理员不能查看租户内用户使用的配置项统计（仅系统管理模块）
10. **配置项的审计日志**：只针对特定配置项，管理员、租户记录，用户不记录
11. **配置项的可见性规则**：某些特定的配置项普通用户看不到
12. **用户配置的权限控制**：需要权限控制，租户管理员配置用户可配置（粗项），配置项中要关联角色，及这个配置项哪些角色可以配置
13. **用户配置的查看权限**：用户查看配置项历史不需要权限控制，所有用户都可以查看

## 权限码命名规范

统一遵循 `module:resource:action` 格式：
- 系统管理模块：如 `system:config:read`、`system:config:update`
- 文档管理模块：如 `doc:file:read`、`doc:file:upload`、`doc:folder:create`

## 三种管理角色权限说明

1. **系统管理员**：整个系统就只有一个，作用是超管，配置整个系统的默认配置，创建租户以及租户的账号，只管理租户本身
2. **租户管理员**：配置角色、权限、配置项、创建用户、授权等，只能管理用户账号
3. **普通用户**：上传文档，问答，配置

## 用户配置的统一规则

1. **用户配置存储方式**：A+B方案，每个文件保存自己的配置（文件-配置一对一），用户有"最近使用的配置"作为默认值
2. **用户配置更新**：用户为文件配置后自动更新"最近使用的配置"
3. **文件删除影响**：删除文件时配置也删除，不影响"最近使用的配置"
4. **配置的生效时机**：立即生效
5. **用户配置查看**：文件列表中显示每个文件使用的配置项；用户可以查看自己之前为不同文件配置的配置项历史，不可以修改
6. **用户配置权限**：需要权限控制，租户管理员配置用户可配置（粗项），配置项中要关联角色，及这个配置项哪些角色可以配置；默认所有的用户都可以配置
7. **用户配置查看权限**：用户查看配置项历史不需要权限控制，所有用户都可以查看
8. **配置项的可见性规则**：某些特定的配置项普通用户看不到
